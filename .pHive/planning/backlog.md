...
| Codex CLI install/detection support | `done` | consus-phase21-codex-cli-support, `f756649`, merged to `dev` via PR #111. Resolved with a real, primary-source-confirmed mechanism — Codex's own bundled `skill-installer` system skill documents `$CODEX_HOME/skills/<name>/SKILL.md` (the same filename convention Claude Code uses), verified by directly reading a real Codex CLI installed on this machine, which corrected an earlier web-research-only conclusion that this would need a whole new MCP-server epic. `scripts/agent-init.mjs` generalized to a per-harness table (shared byte-comparison logic, not duplicated); `--harness claude`/`--harness codex` narrows to one. Live-verified against both real installed harnesses on this machine. No MCP server built — Codex's MCP support is real but explicitly out of scope, consistent with Consus's plain-REST-API-plus-skill-file model. |
| Other non-Claude-Code/non-Codex agent harnesses | `backlogged` — needs its own real discovery-mechanism research per harness, same discipline as Codex | not yet investigated |
...
| OSS docs + GitHub Pages site | `done` (mainline's own version) | mainline `docs/PAN-7999-oss-launch-readme-vision`-equivalent work already shipped (README.md, VISION.md exist). archived-dev's `20b05c3` (PAN-7153) is a redundant second implementation — not a gap. The earlier caveat (README/VISION still describing the pre-strip, Multica/Minerva-coupled architecture) is resolved — both, plus `docs/api-reference.md` and `skills/consus/SKILL.md`, were brought current with real v0.9.0/v0.10.0 capability, merged to `dev` via PR #107. |
| Interaction polish / accessibility pass on transitions and interactive states | `done` (the real, verified subset — see below) | consus-phase20-diagram-editor-a11y, merged to `dev` via PR #110. A real audit (not the vague archived-dev placeholder, `1358e9b`/PAN-7763) read the actual shipped consus-phase18/19 code and live-verified findings before any story was written: (1) keyboard-accessible node move (arrow keys, reusing the pointer-drag's own `buildMovedChange`) + edge delete (Enter/Space, reusing the same `onSnipEdge` the click path uses) in `DiagramCanvas.tsx`; (2) a genuine Tab-cycle focus trap + focus-return for `CommandPalette.tsx`, fixing a live-reproduced bug (`role="dialog" aria-modal="true"` implemented no real containment — Tab escaped to an unrelated page element); (3) `prefers-reduced-motion` support for the Harness skin's cursor blink, the one confirmed animation gap (zero-result grep confirmed no other case existed). The audit also confirmed several other surfaces (`ThemeSkinPicker`, `HarnessConnectBanner`, `DiagramSourcePanel`, `DiagramMetadataStrip`, the palette's own list items, connect-mode) are already correctly accessible — not re-touched, not manufactured as findings. |
