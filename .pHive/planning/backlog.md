...
| **In-place doc editor with explicit edit/view mode toggle and a "fire" action distinct from "save"** (`DocEditor.tsx`, `updateDoc`/`fireDoc`, `PUT`/`GET /api/docs/content`, a dedicated fire endpoint) | `backlogged` | archived-dev, `0302558` (PAN-8237), `d6e748d`/`b7a1f8c` (PAN-8235/8224), `9ab4185` (PAN-8236 doc fire endpoint). Real, working UI — closer to what "open and interact... fire off work" actually means than the current diff-compose box. Needs decoupling from its Minerva-park-and-resume plumbing before porting. |
...
| Generic "push a decision/CBA into Consus" HTTP endpoint — today only Consus's own internal flows can create an item with a `decision_payload`; no outside agent/harness can | `planned-not-built` | discussed tonight (this session's conversation), grounded in a real gap confirmed by reading `skills/consus/SKILL.md` (read/verdict only, no create) and `server/routes/*.ts` (no POST that creates a decision item). Not yet turned into a formal `.pHive/epics/` entry as of this writing — the branch-reconciliation work interrupted planning it. Next step when picked back up: run `/plugin-hive:plan` for it specifically. |
