...
- **`decision-request/v1` real source not yet fetched** — prior-art.md calls for porting `decision-request.ts` "nearly verbatim" from the hive host (`ssh <internal-dev-host>` → `Claud-ometer/src/lib/consus/decision-request.ts`); this reconciliation pass did not fetch it. Recommended before `decision-request-v1-contract` implementation starts — building purely from prior-art.md's summary risks drifting from the actual proven shape.
