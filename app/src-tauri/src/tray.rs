//! Menu-bar tray: Open Consus / Check for Updates… / Launch at Login / Quit.
//! Closing the window hides it (sidecar keeps running, tray icon stays) --
//! standard menu-bar-app UX, avoids re-paying sidecar startup cost on every
//! reopen. Quit is the one path that must actually kill the spawned sidecar
//! explicitly (see lib.rs's RunEvent::ExitRequested/Exit handler, which this
