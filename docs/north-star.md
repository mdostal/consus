# Consus — Product Spec (seed for kickoff)

_Provisional name "Consus" (renames once it works — don't over-invest in the branding). Seed
this into the consus repo's `docs/north-star.md` when we `/hive:kickoff` it — soon (target: this
...
The orchestrator *runs* the loop; humans *view and decide* through Consus.

## THE ACUTE PAIN (the #1 requirement — this is why Consus exists)
...
box and rendering it as an Artifact (e.g. the pantheon-orchestrator kickoff brief). **Consus is
...
4. **CBA / KB — the "Consus section"** — approved CBAs / docs / decisions become shared-truth KB;
...
6. **Pluggable + swappable** — a Pantheon plugin, its own repo (`mdostal/consus`).

## Bootstrap ordering
Consus fires **once the orchestrator's first slice (P0+P1) is solid** — because the orchestrator is
what runs the loop. Until then, **Mathew + Claude ARE Consus (manual)**. First real package to load:
...
Consus's document/decision rendering surface **should use the artifact-UI pattern prototyped in this session** — clean, **theme-aware** (light/dark), readable HTML surfaces that render generated docs, deep-dives, and go/no-go decisions. See the precedents built by hand tonight: the **CADEX deep-dive**, the **gig go/no-go**, and the **Consus/Heimdall setup** artifacts.

**Those artifacts ARE the manual Consus output** — hand-rendered only because Consus doesn't exist yet. Consus productizes exactly that: every generated artifact + every decision rendered as a **readable, navigable surface** (not a terminal, not raw markdown). Baseline UX requirements drawn from those precedents:
