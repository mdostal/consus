...
- **HTTP API** — `GET /health`; `GET`/`POST /api/decisions` (read the open queue, now with
  ...
  links. Full contract in the [API Reference](../api/index.md).
...
New here? Read `.pHive/planning/vision-and-way-of-working.md` and `.pHive/planning/backlog.md` in the repo for the fuller current-state picture, then see the [API Reference](../api/index.md) for the HTTP contract. Land a test with your change.
